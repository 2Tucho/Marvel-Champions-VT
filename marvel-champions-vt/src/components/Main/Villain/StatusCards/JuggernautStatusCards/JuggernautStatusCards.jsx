...
function JuggernautStatusCards () {
  const { setToughStatus, setConfusedStatus, setStunnedStatus } = useContext(StatusCardsContext);
  const { setMomentumCounters } = useContext(MomentumCountersJuggernautContext);
  ...
  return <>
    <article>
      ...
      <button onClick={() => setMomentumCounters(0)}>Remove counters</button>
    </article>
